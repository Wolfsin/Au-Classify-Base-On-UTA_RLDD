...
def DataExpansion(data, eachSampleFrame=150, interval=20, splitFlag=False, label=None, mixFlag=False):
    """
    ...
    if mixFlag:
        ...
        #     if i == 0:
        #         plt.plot(range(expansionData.shape[0]), expansionData[:, 0],linestyle='solid', color='red',label='train')
        #     elif i == 16:
        #         plt.plot(range(expansionData.shape[0]), expansionData[:, 0],linestyle='solid', color='blue',label='val')
        #     elif i ==18:
        #         plt.plot(range(expansionData.shape[0]), expansionData[:, 0],linestyle='solid', color='green',label='test')
        #     resList.append(expansionData)
        # plt.legend()
        # plt.show()
        for i in range(data.shape[0]//eachSampleFrame):
            supData = data[i*eachSampleFrame:(i+1)*eachSampleFrame, :]
            # plt.plot(range(150), supData[:, 13],linestyle='solid')
            # plt.show()
            resList.append(supData)

        if data.shape[0]%eachSampleFrame != 0:
            supData = data[data.shape[0]//eachSampleFrame*eachSampleFrame:, :]
            resList.append(supData)

    if splitFlag:
        if not mixFlag:
            if splitMethod == 'positive':
                trainList = resList[:int(len(resList) * 0.7)]
                trainList, valList = train_test_split(trainList, test_size=0.2, random_state=42)
        ...
        plt.plot(range(trainList[0].shape[0]), trainList[0][:, 0], linestyle='solid', color='green', label='train')
        plt.plot(range(valList[0].shape[0]), valList[0][:, 0], linestyle='solid', color='blue', label='val')
        plt.plot(range(testList[0].shape[0]), testList[0][:, 0], linestyle='solid', color='red', label='test')
        plt.legend()
        plt.show()
        ...
        plt.plot(range(resList[0].shape[0]), resList[0][:, 0], linestyle='solid')
        plt.legend()
        plt.show()
        return resList
...
def GenerateInputSet(Path=r'D:/UTA Real-Life Drowsiness Dataset AU Preprocessing/Group{}/', LoadWithSplit=False):
    ...
    if LoadWithSplit:
        trainSet = pad_sequences(trainSet, maxlen=150, padding="post", truncating="pre", value=-99, dtype='float32')
        valSet = pad_sequences(valSet, maxlen=150, padding="post", truncating="pre", value=-99, dtype='float32')
        testSet = pad_sequences(testSet, maxlen=150, padding="post", truncating="pre", value=-99, dtype='float32')
    else:
        ...
        for index, sample in enumerate(TestList):
            padSample = pad_sequences(sample, maxlen=900, padding="post", truncating="pre", value=-99, dtype='float32')
            TestList[index] = padSample
            testSet = concatDF(testSet, padSample)
    ...
    np.save(r'D:/UTA Real-Life Drowsiness Dataset AU Preprocessing/Train&Test Set/testList.npy', TestList)
...
def CreateModel(sequenceLength):
    ...
    model.add(LSTM(units=256, kernel_initializer='Orthogonal', return_sequences=False))
    model.add(Dropout(0.5))
    # model.add(TimeDistributed(Dense(3, activation='softmax')))
    # model.add(LSTM(units=128, kernel_initializer='Orthogonal', return_sequences=False))
...
# hist = LSTMModel.fit(trainX, trainY, validation_data=(valX, valY), batch_size=900, epochs=100, shuffle=True,
#                      verbose=1)
